...
describe('Data API - verification', function() {
  ...
  it('should verify a valid signed DataItem', async function() {
    ...
    const verified = await Data.verify(signed, { n: wallet0.n } as any);
    
    expect(verified).to.equal(true);
  })

  it('should fail to verify a DataItem with the wrong public key', async function() {
    
    const item0 = await Data.create({ data: 'TESTSTRINGA' }, wallet0);
    const signed = await Data.sign(item0, wallet0)
    
    expect(
      await Data.verify(signed, wallet1)
    ).to.equal(false);

    expect(
      await Data.verify(signed, { n: '' }  as any )
    ).to.equal(false);
    
    expect(
      await Data.verify(signed, { n: INVALID_BASE64U }  as any )
    ).to.equal(false);
  ...
  it('should fail to verify a DataItem with no signature or id', async function() {
    ...
    expect(
      await Data.verify(item0, wallet0)
    ).to.equal(false);
    ...
    expect(
      await Data.verify(signed, wallet0)
    ).to.equal(false);
    ...
    expect(
      await Data.verify(signed2, wallet0)
    ).to.equal(false);
  })
