...
export async function unbundleData(
  ...
  if (
    ...
    console.warn(
      `Invalid bundle, should be a json string or object with an items Array`
    );
    return [];
  }
  ...
  const verifications = await Promise.all(
    itemsArray.map((d) => verify(deps, d))
  );
  const failed = verifications.filter((v) => !v).length;
  if (failed > 0) {
    console.warn(
      `${failed} pieces of Data failed verification and will be discarded`
    );
