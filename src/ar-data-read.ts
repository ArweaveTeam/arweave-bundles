 ...
 * @param tag
 */
export async function decodeTag(
  deps: Dependencies,
 ...
 *
 */
export async function decodeTagAt(
  deps: Dependencies,
  d: DataItemJson,
  index: number
) {
  if (d.tags.length < index - 1) {
    throw new Error(
      `Invalid index ${index} when tags array has ${d.tags.length} tags`
    );
  }
 ...
 * @param d
 */
export async function unpackTags(
  deps: Dependencies,
  d: DataItemJson
  ...
  for (const tag of d.tags) {
    const { name, value } = await decodeTag(deps, tag);
