...
import {
  verifyEncodedTagsArray,
  MAX_TAG_COUNT,
  MAX_TAG_KEY_LENGTH_BYTES,
  MAX_TAG_VALUE_LENGTH_BYTES,
} from "./ar-data-verify";
...
export async function createData(
  ...
  if (!verifyEncodedTagsArray(deps, d.tags)) {
    throw new Error(
      `Tags are invalid, a maximum of ${MAX_TAG_COUNT} tags, a key length of ${MAX_TAG_KEY_LENGTH_BYTES}, a value length of ${MAX_TAG_VALUE_LENGTH_BYTES} has been exceeded, or the tags are otherwise malformed.`
    );
  }
